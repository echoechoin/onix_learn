...
#define KEYBOARD_DATA_PORT 0x60 // 读：键盘扫描码
#define KEYBOARD_CTRL_PORT 0x64 // 写：命令/数据 读：状态寄存器

#define INV 0 // 不可见字符
#define CODE_PRINT_SCREEN_DOWN 0xB7

#define KEYBOARD_CMD_LED 0xED // 设置 LED 状态的命令
#define KEYBOARD_CMD_ACK 0xFA // ACK 表示命令执行成功后，读取KEYBOARD_CTRL_PORT端口会返回的值
...
static char keymap[][4] = {
    // ext == 2 表示普通按键
    // ext == 3 表示扩展按键 也可以区分ctrl/alt等按键左右的区别
    /* 扫描码 | 未与 shift 组合 | 与 shift 组合 | ext == 2 | ext == 3 */
    /* ---------------------------------- */
    ...
    /* 0x1B */ {']', '}', false, false},
    /* 0x1C */ {'\n', '\n', false, false}, // ENTER
    /* 0x1D */ {INV, INV, false, false},   // CTRL_L 左：0x1D 右：0xe0 0x1D
    ...
    /* 0x38 */ {INV, INV, false, false},  // ALT_L 左：0x38 右：0xe0 0x38
...
static lock_t lock;    // 锁，在
static task_t *waiter; // 等待输入的任务

#define BUFFER_SIZE 64        // 输入缓冲区大小，表示最多可以输入 64 个字符，之后的字符将被丢弃
static char buf[BUFFER_SIZE]; // 输入缓冲区
static fifo_t fifo;           // 存储键盘输入字符的循环队列
...
static void keyboard_wait()
{
    ...
    } while (state & 0x02); // 读取键盘输入缓冲区，直到为空
    // state：状态寄存器：

    // 0. 输出缓冲区状态：1 表示输出缓冲区满
    // 1. 输入缓冲区状态：1 表示输入缓冲区满 **只关心这个位**
...
// 直到键盘返回 ACK/RESEND，但是为了简单起见，这里不处理RESEND
// ACK表示键盘正确接收到上一条命令，不需要重新发送
// RESEND表示键盘没有正确接收到上一条命令，需要重新发送
...
void keyboard_handler(int vector)
{
    ...
    // 是扩展码字节 扩展按钮会先发送一个0xe0再发送扫描码
    if (scancode == 0xe0)
    {
        // 置扩展状态 
        extcode_state = true;
        // 等待下一个扫描码（有扩展码的按键会发生两次中断）
        return;
    }

    // 是扩展码的话，使用第4个字节表示按键是否被按下
    if (extcode_state)
    {
        // 改状态索引
        ext = 3;
    ...
    // 因为 Print Screen 的通码是 0xB7，
    // 但是为了和keymap对应，我们把它定义为 0x5F(因为5f~b7是空的，为了节约空间)
    // 这样就可以通过keymap的索引来访问了
    ...
    // 计算 shift 状态
    bool shift = false;
    // 如果是字母键，且 capslock 开启，则 shift 状态取反，之后输出的字母大小写会反转
